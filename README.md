...
> 
> Another option: 層構成 (soukousei) - layer composition

The primary intention of the library is to make configuration simple and reliable. It gives a way to compose a struct from multiple layers in a customisable, modular and non-boilerplate way. Main construction ways are deserialization (`serde`) and ENV loading[^1] (or simply loading from a key-value storage). All of this should be covered with great error reporting, because the configuration part usually is a part of public API.

## Design

This library has (wip) a macro to generate a partial of a struct. Let's call it `Partial`:

```rust
use std::num::NonZeroU64;
use totonoeru::Partial;
...
This section is raw. Currently, you can always use bare partials:
...
```rust
use totonoeru::{Source, source::{Toml, Json, TryMultiple}};
...
- `derive` feature flag for `totonoeru_derive` re-export (e.g. `serde`, `clap`)? TODO: read about idiomatic usage of feature flags in general.
...
- Advanced merge strategies, when it might depend on the state of other fields? Overkill and out of scope of totonoeru, IMO.
...
- [ ] A field-level `parse_env`[^1] attribute to override how fetched ENV var is parsed (default is `FromStr::from_str`). **Could be manually implemented with a custom `Partial`.**
- [ ] A field-level `merge` attribute to customise the default "replace with newer" strategy. **Could be manually implemented with a custom `Partial`.**
...
- [ ] Research in the [Compiler Explorer](https://godbolt.org/) how does wrapping ALL fields into partials affects compiled code.
...
[^1]: I am thinking more and more about moving `from_env` out of `trait Partial`. Generalise it to `from_map`?